...
namespace RegexViewer
{
    ...
    //                FilterFileItem fileItem = (FilterFileItem)(sender as ItemCollection).CurrentItem;
    //                foreach (var item in (sender as ItemCollection))
    //                {
    //                    SetStatus(item.ToString());
    //                    if (!(item is FilterFileItem))
    //                    {
    //                        // remove temp entry
    ...
    //                //FilterFileItem item = (FilterFileItem)(items.Last());
    //                //item.Index = this.ContentList.Max(x => x.Index) + 1;
    //                // FilterFileItem fileItem = new FilterFileItem();
    //                //fileItem.Index = this.ContentList.Max(x => x.Index) + 1;
    //                //grid.Items.Add(fileItem);
    //                // grid.SelectedIndex = grid.Items.Count - 1;
    //                // this.ContentList.Add(fileItem);
